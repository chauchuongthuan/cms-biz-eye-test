...
export class TagComponent implements OnInit {
  ...
  // public listOfData = [];
  @ViewChild('editForm') editForm: CreateEditTagComponent;
  ...
  getTag(params: HttpParams) {
    ...
    this.categoryService.getTag(params).subscribe(data => {
      this.listTag = data.list;
      this.paginator = {
        ...data.paginator,
        option
      }
  ...
  onDelete(id: string){
    this.tableLoading = true
    this.categoryService.deleteTag(id).subscribe(data => {
        this.getTag(this.params)
      },
  ...
  onExport(){
    this.categoryService.exportTag(this.params)
  }
