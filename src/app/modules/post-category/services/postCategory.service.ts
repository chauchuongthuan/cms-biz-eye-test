...
export class CategoryService {
  ...
  getCategory(params: HttpParams) {
    return this.apiService
      .get<ICategory>(environment.BASE_URL + 'admin/post-categories', this.auth.getHeaderAuth(), params)
      .pipe(
  ...
  exportCategory(params: HttpParams) {
    let exportUrl = environment.BASE_URL + 'admin/post-categories?' + params.toString() + `&token=${this.auth.getTokenUser()}&get=true&export=true`;
  ...
  editCategory(data: any, id: string) {
    return this.apiService.putFile(
      environment.BASE_URL + 'admin/post-categories/'+id,
      data,
      this.auth.getHeaderAuth({}),
    ).pipe(
      catchError((error) => this.apiService.handleErrorObservable(error, this.notification)),
