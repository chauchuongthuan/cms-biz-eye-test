...
export class MailListComponent implements OnInit {
   @ViewChild("actionComponent") actionComponent: CreateEditMailListComponent;
   ...
   getMailList(params: HttpParams) {
      ...
      this.mailListService.getMailList(params).subscribe((data) => {
   ...
   onDelete(id: string) {
      this.tableLoading = true;
      this.mailListService.deleteMailList(id).subscribe(
         (data) => {
            this.getMailList(this.params);
         },
   ...
   exportExcel() {
      this.mailListService.exportMailList(this.params);
   }

   importInsurance() {
      this.importComponent.showLarge();
   }

   changeStatus(id: any, $event: any) {
      this.tableLoading = true;
      this.mailListService.changeStatus(id, $event).subscribe(
         (data) => {
            this.getMailList(this.params);
         },
         (error) => (this.tableLoading = false),
