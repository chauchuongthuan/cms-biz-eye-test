...
export class CreateEditContactComponent implements OnInit {
  ...
  formControl() {
    return this.fb.group({
      ...
      messageFile: new FormControl({ value: '', preview: null }, []),
   });
  }
  ...
  dataInit(data: IContact, action: boolean){
    ...
    if(data.messageFile){
      this.messageFile = [{ value: '', preview: data.messageFile }];
    }
    else {
      this.messageFile = []
    }
    this.contactForm = this.fb.group({
      ...
      messageFile: new FormControl({ value: '', preview: data.messageFile }, []),
    });
    if(data.messageFile)
      this.messageFile = [{ value: '', preview: data.messageFile }];
