...
export class EditorComponent implements OnInit {
   ...
   ngOnInit(): void {
      ...
      if (param.id) {
         // this.isEdit = true;
         this.postService.getDetail(param.id).subscribe(
            (data: any) => {
               this.initData(data);
            },
            (error) => {
   ...
   getPostDetail() {
      this.postService.getDetail(this.id).subscribe((data) => {
   ...
   onSEO() {
      ...
      this.postService.chatGPT(formData).subscribe(
         (data: any) => {
      ...
      if (this.id)
         this.postService.editPost(formData, this.id).subscribe(
            (data) => {
               ...
               else this.router.navigateByUrl("/admin/posts");
            },
            (error) => {
               this.isLoading = false;
            },
