...
export const isFileObject = (object: any) => {
   ...
   return keys.includes("value") && keys.includes("preview") && (isFile(object["value"]) || !object["value"]);
};
...
const convertObjectDataToForm = (formData: FormData, field: any, object: Object, fileFields: Array<any> = []) => {
   ...
   keys.forEach((key, index) => {
      ...
      if (["string", "boolean", "number"].includes(typeof values[index])) {
         ...
         convertArrayDataToForm(formData, appendField, values[index], fileFields);
         ...
         if (isFileObject(values[index])) {
            ...
            convertObjectDataToForm(formData, appendField, values[index], fileFields);
         }
      }
   });
};

const convertArrayDataToForm = (formData: FormData, field: any, array: Array<any>, fileFields: Array<any> = []) => {
   array.map((value: any, index) => {
      ...
      if (isFileObject(value)) {
         let file = value.value ? value.value : getFileNameFromPath(value.preview || "");
         convertSimpleDataToForm(formData, `${field}[${index}]`, file);
      } else if (["object"].includes(typeof value)) convertObjectDataToForm(formData, `${field}[${index}]`, value, fileFields);
   });
};
