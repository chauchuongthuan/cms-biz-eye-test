...
export class DigitopUploadComponent implements OnInit {
  ...
  beforeUpload = (file: NzUploadFile, _fileList: NzUploadFile[]): Observable<boolean> =>
    new Observable((observer: Observer<boolean>) => {
      const isJpgOrPng = file.type === 'image/jpeg' || file.type === 'image/png' || file.type === 'image/jpeg' || file.type === 'xlsx';
      if (!isJpgOrPng) {
      ...
      this.getBase64(file, (img: string) => {
        ...
        }else this.changeFile.emit(this.fileList[0] ? this.fileList[0] : { value: '', preview: null })
      });
    });
